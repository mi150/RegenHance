...
class EfficientDet(DNN):
    ...
    def inference(self, image, detach=False, grad=False):
        ...
        ori_imgs, x, framed_metas = preprocess_accmpeg(image, max_size=1920)
        x = torch.stack(x)
...
def preprocess_accmpeg(images, max_size=1920):
    imgs_meta = [
        aspectaware_resize_padding(image, 1920, 1920, means=None)
        for image in images
    ]

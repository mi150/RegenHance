...
sr_images_direc = '/YOUR_dataset/sr_images/'   ## please replace your path
o_images_direc='/YOUR_dataset/origin_images/'  ## please replace your path
...
def get_groundtruths(args, train_val_set, path, visualize_step_size, tag):
    ...
    for data in loader:
        ...
        #mask_grad = sr_image.grad
        #print(mask_grad_.shape)
        #print(mask_grad_complete.shape)
        ...
        mask_grad = F.conv2d(
            mask_grad_,
            torch.ones([1, 1, 24, 24]).cuda(),
            stride=24,
        )
        
        #mask_grad = mask_grad_
        ...
        # mask_grad = mask_grad.detach().cpu()
        #print(mask_grad.shape)
        #VIS_GT[real_id,:,:,:] = mask_grad
        #print(VIS_GT[real_id,:,:,:])
...
def main(args):
    ...
    if args.training_set == "COCO":
        ...
        train_val_set = Test(root='/data01/wangweijun/accsr/AccMPEG/')
        logger.info("Dataset size: %d", len(train_val_set))
        training_set, cross_validation_set = torch.utils.data.random_split(
            train_val_set,
            [
    ...
    for iteration in range(args.num_iterations):
        ...
        for idx, data in enumerate(training_loader):
            ...
            #print(sr_image.shape)
            #print(bit_stream.shape)
            #target = F.interpolate(target, size=[45, 80], mode="bicubic") 
            with torch.enable_grad():
                #temp = sr_image.cuda()
                temp = o_image.cuda()
                if temp.shape[0] != 4:
        ...
        for idx, data in enumerate(cross_validation_loader):
            ...
            with torch.no_grad():
                ...
                # set_trace()
                #mask_slice = mask_generator(sr_image.cuda())

...
class Video(Dataset):
    def __init__(self, video, postprocess, logger, return_fid=False):
        self.video = video
        logger.info(f"Extract {self.video} to pngs.")
        self.path = "/data02/wangweijun/RegenHance/or_images"   ## please replace your dataset path
        subprocess.run(["rm", f"{self.path}/*.png"], stderr=subprocess.DEVNULL)
        subprocess.check_output(
            [
                ...
                "-start_number",
                "0",
                f"{self.path}/%010d.png",
            ]
        )
        self.nimages = len(glob.glob(f"{self.path}/*.png"))
    ...
    def __getitem__(self, idx):
        image = T.ToTensor()(plt.imread(f"{self.path}/%010d.png" % idx))

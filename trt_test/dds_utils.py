...
def cal_miou(truth, pred, labels, n_classes=3):
    ...
    for i in range(n_classes):
        TP=matrix[i][i]
        FN=matrix.sum(axis=1)[i]-matrix[i][i]
        FP=matrix.sum(axis=0)[i]-matrix[i][i]
        iou[i]=TP/(TP+FN+FP)
...
def filter_results(bboxes, gt_flag, gt_confid_thresh, mpeg_confid_thresh,
    ...
    for b in bboxes:
        ...
        if w*h <= max_area_thresh and label in relevant_classes:
            result.append(b)
    return result
...
def evaluate(min_fid,max_fid, map_dd, map_gt,
    ...
    # if tp+fp==0:return (tp, fp, fn, count,
    #         0,
    #         0,
    #         0)

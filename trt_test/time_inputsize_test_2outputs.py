...
def init():
    ...
    model_path ="./InputSizeEDSR/EDSR_256.engine"
    model_path ="/home/ubuntu/csw/PaddleSeg/yolo/YOLOX-main/yolox_x.engine"
    ...
    print("输出",engine.get_binding_shape(2))
    ...
    h_output1 = cuda.pagelocked_empty(abs(trt.volume(engine.get_binding_shape(2))), dtype=trt.nptype(trt.float32))    
    h_output2 = cuda.pagelocked_empty(abs(trt.volume(engine.get_binding_shape(1))), dtype=trt.nptype(trt.int32))
    ...
    return context, h_input, h_output1, h_output2, stream, d_input, d_output1, d_output2, sumtime, t_to_cuda_time, infer_time, t_to_cpu_time

# 推理
def inference(data_path):
    global context, h_input, h_output1, h_output2, stream, d_input, d_output1, d_output2, sumtime, t_to_cuda_time, infer_time, t_to_cpu_time
    ...
    # 模型预测
    t1 = time.time()
    context.execute_async(batch_size=32,bindings=[int(d_input), int(d_output1), int(d_output2)], stream_handle=stream.handle)
    stream.synchronize()
    t2 = time.time()
    print("推理时间", t2 - t1)
    ...
    print(h_output1.shape)
    print(h_output2.shape)
    #print(np.max(h_output), np.min(h_output))
    h_output1 = np.reshape(h_output1, (100, 5))
    h_output2 = np.reshape(h_output2, (100))
...
if __name__ == '__main__':
    context, h_input, h_output1, h_output2, stream, d_input, d_output1, d_output2, sumtime, t_to_cuda_time, infer_time, t_to_cpu_time = init()
    ...
    for i in range(500):
        ...
        output, output2 = inference(data_path="")
        print(output)
        print(output2)
